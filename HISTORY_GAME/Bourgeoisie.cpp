/*****************************************************************************
 * File Name: Bourgeoisie.cpp
 * Programmer: Glen Gougeon
 * Last-Mod: 9-4-20
 * Game Title: A Nap in Time, A History Major's Unexpected Journey

 * Description:		Definitions for Bourgeoisie SUB Class :
...
// ROLL : INITIATIVE
int Bourgeoisie::rollInitiative()
{
	printEnterWipe("Roll for initiative..");

	// uses 1d12 to roll initiative
	int roll = initiative.twelve();
 ...
 * *****************************************************************/
int Bourgeoisie::attack(Character *enemy)
{
 ...
 * *****************************************************************/
void Bourgeoisie::defend(int harm)
{
	// roll defense & record
...
Description:
	...
	STEPS:
		...
		3.  If step 2 successful
				a) roll healing die
				b) restrict maximum outcome to BOURGEOISIE_MAX_HEALING
				c) add b) to current strength
				d) print previous and current strength

*******************************************************************/
void Bourgeoisie::health()
{
	printEnterWipe("Help.. Help.. I am injured!");
	printEnterWipe("I have done all for the love of France.. help me!");

	if (this->rollInitiative() >= 4)
	{
		int healthBoost = heal.eight();
		if (healthBoost >= BOURGEOISIE_MAX_HEALING) { healthBoost = BOURGEOISIE_MAX_HEALING; }
