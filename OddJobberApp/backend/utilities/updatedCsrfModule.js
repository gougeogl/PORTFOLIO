/*********************************************
 ◘ Note:
  The original module required updates which
  I have noted in the code below. 

 ◘ Updates:
   • Array.prototype.substr is deprecated
     replaced with Array.prototype.substring

   • hash algoriths sha1 replaced with more
     secure sha256 algorithm

   • value
    A custom member (or function) that can
    be added to the module as an option
 *********************************************/
...
Tokens.prototype.verify = function verify (secret, token) {
  ...
  var salt = token.substring(0, index) // original used deprecated substr function
...
function hash (str) {
  return crypto
    .createHash('sha256') // original used sha1
    .update(str, 'ascii')
    .digest('base64')
